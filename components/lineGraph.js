...
export default class LineGraph extends React.Component {
  ...
  pointsToPathString() {
    ...
    var distances = points.map(e => e[2]);
    ...
    var relativePoints = points.map((point) => [((maxElevation - point[0])/elevationWindow), (point[2]/fullDistance)]);

    return relativePoints.reduce((a,p,i) => a + `${p[1] * 200},${p[0] * 100} `, "0,100 ") + "200,100";
  }

  render() {
    if (!this.props.points || !this.props.points.length) return null;
