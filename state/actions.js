...
const selectTrail = ({ properties, geometry }) => {
  return (dispatch, getState) => {
    ...
    if (!cachedTrail || !cachedTrail.satelliteImageRequested)
      dispatch(
        getSatelliteImage({ id: properties.id, center, zoom, reducer: "trail" })
      );
...
const selectBoundary = ({ properties, geometry }) => {
  return (dispatch, getState) => {
    ...
    if (!cachedBoundary || !cachedBoundary.satelliteImageRequested)
      dispatch(
        getSatelliteImage({
          id: properties.id,
          center,
...
const getSatelliteImage = ({ id, center, zoom, reducer }) => {
  return dispatch => {
    dispatch({
      type: `SET_${reducer.toUpperCase()}_SATELLITE_IMAGE_REQUESTED`,
      id
    });
    ...
    return fetchWithCache({
      ...
      .then(image => {
        return dispatch({
          type: `SET_${reducer.toUpperCase()}_SATELLITE_IMAGE`,
          id,
          center,
          zoom,
